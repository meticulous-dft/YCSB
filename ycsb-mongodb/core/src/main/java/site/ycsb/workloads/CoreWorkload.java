 ...
 *
 * <p>Properties to control the client:
 *
 * <UL>
 ...
 *       operate on - uniform, zipfian, hotspot, sequential, exponential or latest (default:
 *       uniform)
 ...
 *       padding the record number. Controls the number of 0s to use for padding. (default: 1) For
 *       example for row 5, with zeropadding=1 you get 'user5' key and with zeropading=8 you get
 *       'user00000005' key. In order to see its impact, zeropadding needs to be bigger than number
 *       of digits in the record number.
...
public class CoreWorkload extends Workload {
   ...
   *
   * <p>If "uniform", "zipfian" or "constant", the maximum field length will be that specified by
   * the fieldlength property. If "histogram", then the histogram will be read from the filename
  ...
  private String buildKeyNamePrefix() {
    return location.isEmpty() ? "" : location + "-";
  }

  public static String buildKeyName(
      long keynum, int zeropadding, boolean orderedinserts, String prefix) {
    if (!orderedinserts) {
  ...
  public void init(Properties p) throws WorkloadException {
    ...
    readallfieldsbyname =
        Boolean.parseBoolean(
            p.getProperty(
                READ_ALL_FIELDS_BY_NAME_PROPERTY, READ_ALL_FIELDS_BY_NAME_PROPERTY_DEFAULT));
    writeallfields =
        Boolean.parseBoolean(
    ...
    if (requestdistrib.compareTo("uniform") == 0) {
      ...
      keychooser =
          new HotspotIntegerGenerator(
              insertstart, insertstart + insertcount - 1, hotsetfraction, hotopnfraction);
    } else {
    ...
    shardKey = p.getProperty("mongodb.shardKey", "");
    location = p.getProperty("mongodb.location", "");
  }
  ...
  public boolean doInsert(DB db, Object threadstate) {
    ...
    String dbkey =
        CoreWorkload.buildKeyName(keynum, zeropadding, orderedinserts, buildKeyNamePrefix());
    ...
    do {
      ...
      if (++numOfRetries <= insertionRetryLimit) {
        ...
        System.err.println(
            "Error inserting, not retrying any more. number of attempts: "
                + numOfRetries
                + "Insertion Retry Limit: "
                + insertionRetryLimit);
        break;
      }
    } while (true);
